...
class HandlerBoxPlot(HandlerBase):
    # Code taken from:
    # https://stackoverflow.com/questions/59274981/can-i-make-a-plot-legend-show-box-plots-using-matplotlib
...
def get_results_mode(mode, exp_cond):
    ...
    for train_ds in train_set_list:
        ...
        for test_ds in list_ds:
            ...
            # name_str = 'RESULT_%s_TRAIN_%s_TEST_%s_%s.pkl' % (mode,
            #                                                   train_ds,
            #                                                   test_ds,
            #                                                   exp_cond)
            ...
            # assert len(name_str) == 1, 'muliple files of the same variation'
            if len(name_str) != 1:
...
if __name__ == '__main__':
    ...
    for ii, ds_str in enumerate(list_ds):
        ...
        if type(temp['one_vs_one'][0]) == list:
            result_dict['one_vs_one'][ds_str] = np.nan #temp['one_vs_one'][0]
        else:
            result_dict['one_vs_one'][ds_str] = temp['one_vs_one'][0].tolist()

        if type(temp['all-one_vs_one'][0]) == list:
            result_dict['all-one_vs_one'][ds_str] = np.nan #temp['all-one_vs_one'][0]
        else:
        ...
        if type(temp['all_vs_one'][0]) == list:
            result_dict['all_vs_one'][ds_str] = np.nan #temp['all_vs_one'][0]
        else:
