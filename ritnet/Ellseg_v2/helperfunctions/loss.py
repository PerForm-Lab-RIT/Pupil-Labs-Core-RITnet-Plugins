...
def get_com(seg_map, temperature=4):
    ...
    # seg_map: BXHXW - single channel corresponding to pupil or iris predictions
...
def get_uncertain_l1_loss(ip_vector,
    ...
    if weights_per_channel == None:
        weights_per_channel = torch.ones(ip_vector.shape[1], dtype=ip_vector.dtype).to(ip_vector.device)
    else:
    ...
    if torch.any(cond):
        ...
        if do_aleatoric:
            ...
            loss_per_sample = .1*uncertain + loss_per_sample/torch.exp(uncertain)
