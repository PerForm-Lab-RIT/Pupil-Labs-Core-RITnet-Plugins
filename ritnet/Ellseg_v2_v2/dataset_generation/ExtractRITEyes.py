...
parser.add_argument('--which_riteyes_set', type=str, default='s-general',
                    help='s-general, s-natural, s-openeds, s-nvgaze')
parser.add_argument('--path2ds', type=str, help='Path to dataset',
...
print('Extracting RITEyes: {}'.format(args.which_riteyes_set))
...
PATH_DIR = os.path.join(args.path2ds, 'RITEyes', args.which_riteyes_set)
...
def process(args, ds_num, fName):
    ...
    Data, keydict = generateEmptyStorage(name='riteyes-{}'.format(args.which_riteyes_set), subset='riteyes-{}_{}'.format(args.which_riteyes_set, fName))
...
pool = multiprocessing.Pool(process=multiprocessing.cpu_count(), maxtasksperchild=1)

pbar = tqdm.tqdm(total=len(list_ds))

def update_tqdm():
    pbar.update()

for ds_num, fName in enumerate(list_ds):
    pool.apply_async(process, args=(args, ds_num, fName), callback=update_tqdm)
